...
def historyStats_function():
    ...
    result = db_session.query(Summary.Origin, Summary.Destination, Summary.Time, Summary.Weekday, Summary.Avg_Travel_Time, origin, destination).        join(origin, Summary.Origin == origin.READER_ID).        join(destination, Summary.Destination == destination.READER_ID).        filter(Summary.Weekday == weekday, Summary.Time == time, Summary.Year == year)
